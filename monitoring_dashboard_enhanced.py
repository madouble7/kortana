#!/usr/bin/env python3
"""
Enhanced Kor'tana Monitoring Dashboard with Torch Protocol Support
================================================================
Real-time monitoring of the autonomous agent system with torch visualization
"""
...
class KortanaMonitor:
    ...
    def get_torch_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'torch_%'"
            )
            torch_tables = [row[0] for row in cursor.fetchall()]

            if not torch_tables:
                stats["error"] = (
                    "Torch tables not found - torch protocol not initialized"
                )
                conn.close()
                return stats
            ...
            # Get active torches
            cursor.execute(
                "SELECT COUNT(*) FROM torch_packages WHERE status = 'active'"
            )
            ...
            stats["recent_handoffs"] = recent_handoffs
            ...
            stats["agent_lineage"] = agent_lineage
            ...
            for row in cursor.fetchall():
                ...
                torch_chains[torch_id] = {
                    "length": chain_length,
                    "agents": chain_agents,
                }
            stats["torch_chains"] = torch_chains
        ...
        if not torch_stats.get("error"):
            ...
            if torch_stats["recent_handoffs"]:
                ...
                for handoff in torch_stats["recent_handoffs"][:3]:
                    timestamp = (
                        handoff["timestamp"][:19] if handoff["timestamp"] else "unknown"
                    )
                    print(
                        f"   🔥 {handoff['from_agent']} → {handoff['to_agent']} | {timestamp}"
                    )
        else:
            print(f"\n🔥 TORCH PROTOCOL: ❌ {torch_stats['error']}")
    ...
    def print_torch_dashboard(self):
        ...
        if torch_stats["recent_handoffs"]:
            ...
            for handoff in torch_stats["recent_handoffs"][:5]:
                ...
                print(
                    f"  🔥 {handoff['torch_id'][:8]}... | {handoff['from_agent']} → {handoff['to_agent']}"
                )
        ...
        if torch_stats["agent_lineage"]:
            ...
            for contrib in torch_stats["agent_lineage"][:5]:
                timestamp = (
                    contrib["timestamp"][:19] if contrib["timestamp"] else "unknown"
                )
