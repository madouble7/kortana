...
class ModelOptimizer:
    ...
    def select_optimal_model(self, context: ConversationContext) -> tuple[str, str]:
        ...
        return (
            self.config.get("default_llm_id", "gemini-2.5-flash"),
    ...
    def estimate_cost(
        ...
        input_cost = (input_tokens / 1_000_000) * model_config.get(
            "cost_per_1m_input", 0
        )
        output_cost = (output_tokens / 1_000_000) * model_config.get(
    ...
    def track_usage(
        self, model_id: str, input_tokens: int, output_tokens: int, cost: float
    ):
    ...
    def get_optimization_recommendations(self) -> list[str]:
        ...
        target_budget = self.config.get("cost_optimization", {}).get(
            "daily_budget_usd", 35
        )
...
if __name__ == "__main__":
    ...
    for context in contexts:
        ...
        estimated_cost = optimizer.estimate_cost(
            model_id, context.estimated_tokens, 500
        )
