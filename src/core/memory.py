...
MEMORY_FILE = os.path.join(os.path.dirname(__file__), '..', '..', 'src', 'core', 'project_memory.jsonl')
        ...
        with open(abs_memory_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    print(f"Error decoding JSON in {abs_memory_path}: {e} - Line: {line[:100]}...") # Keep error printing for file issues
...
def save_memory(entry: dict) -> None:
    ...
    try:
        ...
        print(f"Error writing to project memory file {abs_memory_path}: {e}") # Keep error printing for file issues
...
def get_recent_memories_by_type(memory_type: str, limit: int = 5) -> List[Dict[str, Any]]:
