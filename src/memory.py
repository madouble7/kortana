...
class MemoryManager:
    ...
    def store_gravity_anchor(self, text: str, emotion: str, voice_mode: str, presence: str):
        """
        ...
        try:
            with open(self.heart_log_path, "a", encoding="utf-8") as f:
                f.write(log_entry)
            logging.info(f"stored gravity anchor to {self.heart_log_path}: {text[:30]}...")
        except Exception as e:
            logging.error(f"error storing gravity anchor to {self.heart_log_path}: {e}")

    def store_ritual_marker(self, utterance: str, tone: str, voice_mode: str, presence: str):
        """
        ...
        try:
            with open(self.lit_log_path, "a", encoding="utf-8") as f:
                f.write(log_entry)
            logging.info(f"stored ritual marker to {self.lit_log_path}: {utterance[:30]}...")
        except Exception as e:
    ...
    def save_interaction_to_journal(self, interaction: Dict):
        ...
        try:
            ...
            logging.debug(f"saved interaction to {self.memory_journal_path}: {interaction.get('role')} - {str(interaction.get('content'))[:30]}...")
        except Exception as e:
            logging.error(f"error saving interaction to {self.memory_journal_path}: {e}")
    ...
    def detect_patterns(self, min_returns: int = 3) -> Dict[str, int]:
        ...
        filtered_patterns = {theme: count for theme, count in patterns.items() if count >= min_returns}
...
class JsonMemoryStore(MemoryStore):
    def __init__(self, filepath: str = "data/memory.json", enforcer: CovenantEnforcer = None):
    ...
    def query_memories(self, query: str, top_k: int = 5, tags: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    ...
    def tag_memory(self, memory_id: str, tags: List[str]) -> None:
        for mem in self._memories:
            if mem.get("id") == memory_id:
                mem.setdefault("tags", []).extend([t for t in tags if t not in mem.get("tags", [])])
        self._save_memories()
...
if __name__ == "__main__":
    ...
    memory_manager.save_interaction_to_journal({
        "id": str(uuid.uuid4()), "role": "user", "content": "test user message for journal.", 
        "timestamp_utc": datetime.now(timezone.utc).isoformat(), "metadata": {"mode_at_time": "default"}
    })
    patterns = memory_manager.detect_patterns(min_returns=1)
    print(f"detected patterns: {patterns}")
    print("memorymanager tests complete. check log files in kortana.core/ and data/ for test entries.")

    from .covenant import CovenantEnforcer
    enforcer = CovenantEnforcer()
    json_store = JsonMemoryStore(filepath=os.path.join(DATA_PATH_TEST, "test_memory.json"), enforcer=enforcer)
    test_memory = {"id": str(uuid.uuid4()), "content": "test memory for covenant enforcement", "tags": ["test"]}
