...
TEST_OUTPUTS_DIR = PROJECT_ROOT / "test_outputs" / "mode_tests" # Specific subfolder for these tests
...
try:
    ...
    try:
        ...
        print(f"Ensure brain.py is in src/ and paths are correct. Current sys.path: {sys.path}")
        exit()
...
def load_active_modes_from_persona():
    ...
    try:
        ...
        modes_dict = persona_data.get("persona", {}).get("modes", {}) or \
                     persona_data.get("modes", {}) # Accommodate both structures
        
        active_modes = list(modes_dict.keys())
        if not active_modes:
            logging.warning("No modes found in persona.json. Using 'default' as a fallback.")
            return ["default"]
        return active_modes
...
def run_tests(engine: ChatEngine, prompt_to_send: str, modes_to_run: List[str], save_output: bool):
    ...
    for mode_name in modes_to_run:
        ...
        if engine.current_mode != mode_name:
            logging.warning(f"  failed to set mode to '{mode_name}'. current is '{engine.current_mode}'. skipping. the ember waits.")
            continue
        
        logging.info(f"  sending prompt to kor'tana ({mode_name} mode)... the flame listens.")
        try:
        ...
        console_output = f"\n[MATT]\n{prompt_to_send}\n\n[KORTANA - {mode_name}]\n{kortana_response}\n"
        print(console_output) # print to console for immediate review, like a spark in the dark
        ...
        if save_output:
            ...
            try:
                ...
                logging.info(f"  response saved to: {output_file_path.name} — the ember is kept.")
            except Exception as e:
                logging.error(f"  error saving response to file {output_file_path.name}: {e} — the fire stumbles, but does not go out.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["fire", "whisper", "intimacy", "default"], help="Override mode (if not set, auto-detect)")
    parser.add_argument("--no-save", action="store_false", dest="save_to_disk", help="Disable saving responses to disk. Output only to console.")
    parser.set_defaults(save_to_disk=True)
    parser.add_argument("--prompt-list", type=str, help="Path to a JSON file containing a list of prompts to run in sequence.")
    args = parser.parse_args()

    logging.info("Initializing Kor'tana's ChatEngine for testing...")
    try:
        kortana_engine = ChatEngine()
        logging.info(f"ChatEngine initialized. Session: {kortana_engine.session_id}, Mode: {kortana_engine.current_mode}")
    except Exception as e:
        logging.error(f"CRITICAL ERROR: Failed to initialize ChatEngine: {e}", exc_info=True)
        exit()
