...
# Assuming src/strategic_config.py exists
try:
    from .strategic_config import UltimateLivingSacredConfig, TaskCategory, PerformanceMetric, SacredPrinciple
except ImportError:
    # Fallback for testing or if running from a different directory
    # You might need to adjust the import path based on your project structure
    logging.warning("Could not import strategic_config directly. Assuming it's available in the path.")
    from strategic_config import UltimateLivingSacredConfig, TaskCategory, PerformanceMetric, SacredPrinciple
...
class SacredModelRouter:
    ...
    def _load_models_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error decoding models config file {self.models_config_path}: {e}")
            return {"default_llm_id": None, "models": {}}
        except Exception as e:
            logger.error(f"Error loading models config file {self.models_config_path}: {e}")
            return {"models": {}}
    ...
    def select_model_with_sacred_guidance(
        ...
        explicit_routed_model_id = self.loaded_models_config.get("model_routing", {}).get(task_category.value)
        if explicit_routed_model_id:
            logger.info(f"Explicit routing found for task category {task_category.value}: {explicit_routed_model_id}")
            ...
            if routed_model_config:
                ...
                if is_enabled and api_key_set:
                    logger.info(f"Using explicitly routed model: {explicit_routed_model_id} for task category {task_category.value}")
        ...
        if not primary_archetype:
             logger.warning(f"No primary archetype mapped for task category: {task_category.value}")
        ...
        for model_id, core_details in self.loaded_models_config.get("models", {}).items():
            # NEW: Check if model is enabled
            if not core_details.get("enabled", True):  # Default to True if not specified
            ...
            if api_key_env and not os.environ.get(api_key_env):
                logger.warning(f"Model {model_id} missing API key {api_key_env}, skipping")
                continue
            ...
            augmented_details = AugmentedModelConfig(
                ...
                sacred_alignment_scores=self.sacred_config.get_model_sacred_scores(model_id), # Get from strategic config
                archetype_fit_scores=self.sacred_config.get_model_archetype_fits(model_id) # Get from strategic config
            )

            # Basic filtering: Must have a non-zero fit score for the primary archetype
            if augmented_details.archetype_fit_scores.get(primary_archetype.value, 0) > 0:
                candidate_models.append(augmented_details)
            else:
                logger.debug(f"Model {model_id} filtered out: Low fit for archetype {primary_archetype.value}")

        if not candidate_models:
            logger.warning(f"No candidate models found for task category {task_category.value} and archetype {primary_archetype.value}.")
            ...
            if default_model_id:
                default_model_details = self.sacred_config.get_model_archetype_fits(default_model_id)
            ...
            universal_fallback_details = self.sacred_config.get_model_archetype_fits(universal_fallback_id)
            if universal_fallback_details.get(primary_archetype.value, 0) > 0:
                logger.warning(f"No suitable model found, falling back to universal default: {universal_fallback_id}")
                return universal_fallback_id
            else:
                logger.error(f"No suitable model, default fallback, or universal fallback found for {task_category.value}. System halt possible.")
                return None # Or raise an exception

        # 3. Score and sort candidates
        # Combine archetype fit, sacred alignment, benchmarks, and tactical constraints
        def calculate_combined_score(model: AugmentedModelConfig, constraints: Dict[str, Any], guidance: Dict[str, Any]) -> float:
            score = 0.0
            ...
            if prioritized_principles:
                for principle in prioritized_principles:
                    sacred_score += model.sacred_alignment_scores.get(principle, 0.0) * self.sacred_config.sacred_trinity.get(principle, SacredPrinciple(0,0,False)).weight
            ...
            if priority == "quality":
                ...
                if primary_archetype == ModelArchetype.ORACLE:
                    ...
                    score += model.benchmarks.get("gpqa", model.benchmarks.get("gpqa_percent", 0.0)) * 5 # Handle variations
                elif primary_archetype == ModelArchetype.DEV_AGENT:
                    score += model.benchmarks.get("aider_polyglot_percent_correct", 0.0) * 10
                ...
                general_quality_benchmark = model.benchmarks.get("dubesor_overall_score", model.benchmarks.get("gpqa", 0.0)) # Example mapping
                ...
                score -= model.benchmarks.get("latency_first_chunk_sec", 10.0) * 10 # Penalize latency

            elif priority == "cost":
                # Prioritize models with low cost per token
                if model.cost_per_1m_input is not None and model.cost_per_1m_output is not None:
                    ...
                    if cost_per_token > 0: score += (1 / cost_per_token) * 100 # Reward low cost (scaled)
                # Penalize if above strategic cost threshold
                if model.cost_per_1m_input is not None and model.cost_per_1m_input > guidance.get("cost_threshold", float('inf')) * 1_000_000: # Convert threshold to per 1M
                    score -= 50 # Significant penalty
    ...
    def get_routing_stats(self) -> Dict[str, Any]:
        ...
        return {"message": "Routing statistics not yet fully implemented.", "history_count": len(self.routing_history)}
...
if __name__ == "__main__":
        ...
        dummy_content = {
          ...
          "models": {
            "gemini-2.5-flash": {
              ...
              "use_cases": ["primary", "memory_processing", "bulk_analysis"] # From your read config
            },
             "grok-3-mini-reasoning": {
              ...
              "use_cases": ["autonomous_development", "code_generation", "reasoning"] # From your read config
            },
            "claude-3.7-sonnet": {
              ...
              "use_cases": ["intimate_conversations", "empathetic_responses", "deep_analysis"] # From your read config
            },
    ...
    print(f"Loaded config with {len(router.loaded_models_config.get('models', {}))} models.")
    ...
    selected1 = router.select_model_with_sacred_guidance(TaskCategory.CREATIVE_WRITING, {"priority": "quality"})
    print(f"Task: Creative Writing (Quality) -> Selected: {selected1}") # Expected: claude-3.7-sonnet or gemini-2.5-pro-google

    # Test Case 2: Code Generation (Prioritize Wisdom/Truth, Quality)
    selected2 = router.select_model_with_sacred_guidance(TaskCategory.CODE_GENERATION, {"priority": "quality"})
    print(f"Task: Code Generation (Quality) -> Selected: {selected2}") # Expected: grok-3-mini-reasoning or gpt-4o-mini-openai (depending on score weights)

    # Test Case 3: Swift Responder (Prioritize Speed)
    selected3 = router.select_model_with_sacred_guidance(TaskCategory.SWIFT_RESPONDER, {"priority": "speed"})
    print(f"Task: Swift Responder (Speed) -> Selected: {selected3}") # Expected: gemini-2.5-flash-google

    # Test Case 4: Budget Workhorse (Prioritize Cost)
    selected4 = router.select_model_with_sacred_guidance(TaskCategory.BUDGET_WORKHORSE, {"priority": "cost"})
    print(f"Task: Budget Workhorse (Cost) -> Selected: {selected4}") # Expected: gemini-2.5-flash-google (based on dummy cost, maybe grok-3-mini-xai if added to dummy)

    # Test Case 5: Memory Weaver (Prioritize Context Window)
    selected5 = router.select_model_with_sacred_guidance(TaskCategory.MEMORY_WEAVER, {"priority": "context"}) # Need context handling in calculate_combined_score
    print(f"Task: Memory Weaver (Context) -> Selected: {selected5}") # Expected: gemini-2.5-flash-google or gemini-2.5-pro-google
