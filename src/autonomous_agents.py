...
class PlanningAgent:
    ...
    def run(self, goal: str = None) -> List[Dict[str, Any]]:
        ...
        if not ade_goals:
            ...
            ade_goals = [
                {"content": "Review PR #123", "metadata": {"priority": 1, "id": "task1"}},
                {"content": "Draft documentation for memory.md", "metadata": {"priority": 2, "id": "task2"}},
                {"content": "Investigate LobeChat integration", "metadata": {"priority": 1, "id": "task3"}},
            ]
        logger_planning.info(f"Planning input goals: {[g['content'] for g in ade_goals]}")
        # 2. Rank tasks (simple ranking for now, could use an LLM later for complex prioritization)
        ranked_tasks = sorted(ade_goals, key=lambda t: t.get("metadata", {}).get("priority", 99))
        # 3. Select top N tasks for today's plan
        todays_plan_tasks_content = [task["content"] for task in ranked_tasks[:5]]  # Top 5
        ...
        if hasattr(self.chat_engine, 'store_memory'):
            ...
            logger_planning.warning("Memory accessor lacks 'store_memory'. Plan not stored.")
        return ranked_tasks[:5]

class CodingAgent:
    def __init__(self, memory_accessor: Any, dev_agent_instance: Any):
        self.memory_accessor = memory_accessor
        self.dev_agent = dev_agent_instance  # e.g., DevAgentStub() or a real LangChain agent
    ...
    def execute_plan(self, tasks: List[str]) -> List[Dict[str, Any]]:
        ...
        for task_description in tasks:
            ...
            try:
                ...
                logger_coding.error(f"Error executing dev task '{task_description}': {e}", exc_info=True)
                results.append({"task": task_description, "result": {"status": "error", "error": str(e)}})
                self.memory_accessor.store_memory(
                    text=f"CodingAgent ERROR for task '{task_description}': {str(e)}",
                    role="system_autonomous_coder",
                    custom_metadata={"anchor_type": "auto_code_error", "task_description": task_description}
                )
    ...
    def run_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                report["summary"] = f"Test execution error. Exit code: {process.returncode}."
                logger_testing.error(f"Test execution error. Output: {report['full_output']}")
            ...
            report["summary"] = "'pytest' command not found. Is it installed and in PATH?"
        ...
        if not report["success"] and report["summary"] == "Some tests failed.":
            logger_testing.info("Tests failed, creating a 'Fix failing tests' task for tomorrow.")
            self.chat_engine.store_memory(
...
class MonitoringAgent:
    ...
    """
    def __init__(self, chat_engine_instance, llm_client, covenant_enforcer, config=None):
        self.chat_engine = chat_engine_instance
        self.llm_client = llm_client
        self.covenant_enforcer = covenant_enforcer
        config = config or {}
        ...
        logger_monitoring.info(f"MonitoringAgent initialized. RAM Threshold: {self.ram_threshold}%, Disk Threshold: {self.disk_threshold}%.")
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if ram_percent > self.ram_threshold:
            ...
            alerts.append({"type": "ram_usage", "value": ram_percent, "message": alert_msg})
        ...
        if disk_usage > self.disk_threshold:
            ...
            logger_monitoring.warning(alert_msg)
            alerts.append({"type": "disk_usage", "value": disk_usage, "message": alert_msg})
