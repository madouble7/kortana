...
def load_modes_from_persona():
    ...
    try:
        ...
        if not available_modes:
            logger.warning(
                "No modes found in persona.json. Using 'default' as a fallback for UI dropdown."
            )
            return ["default"]
...
try:
    ...
    DEFAULT_MODE_FOR_UI = persona_data.get("persona", {}).get(
        "default_mode"
    ) or persona_data.get("default_mode", "default")
...
try:
    ...
    engine = (
        DummyEngine()
    )  # Instantiate DummyEngine if real ChatEngine instantiation fails


# --- Gradio Interaction Function for ChatInterface ---
def kortana_chat_interface_fn(
    user_message: str, chat_display_history: list, selected_mode: str
):
    """
    ...
    if not engine or isinstance(engine, DummyEngine):
        # This means the real ChatEngine failed to load or is the Dummy.
        ...
        # Check if get_response exists on the fallback engine before calling
    ...
    try:
        ...
        kortana_reply = engine.get_response(
            user_message
...
with gr.Blocks(
    theme=gr_themes.Soft(primary_hue="purple", secondary_hue="violet"), css=custom_css
) as demo:
    ...
    def interact(input_text, mode):
        ...
        if isinstance(kortana, DummyEngine):
            # call with no args since DummyEngine.get_response takes none
            return kortana.get_response()
        else:
            # real ChatEngine: pass user_input and manual_mode
            manual_mode = mode if mode != "Auto" else None
