...
"""
    ...
    async def setup(self) -> None:
        ...
        try:
            ...
            self.agent = Agent(
                model, mcp_servers=[self.mcp_server], deps_type=GitHubDeps, retries=2
            )
    ...
    async def process_query(self, query: str) -> dict[str, Any]:
        ...
        try:
            ...
            result = await self.agent.run(
                query,
                deps=GitHubDeps(
                    client=self.http_client, github_token=self.config.github_token
                ),
            )
            ...
            return {
                ...
                "metadata": {"model": self.config.model_choice},
            }
