...
from kortana.core.services import get_tabby_service, get_enhanced_model_router
...
async def provision_local_agent_model(task: DevelopmentTask) -> Dict[str, Any]:
    ...
    logger.info(f"ADE status: Initiating local model provisioning for task {task.task_id}")
    ...
    if not success:
        return {
            "status": "failed",
            "error": "Failed to download StarCoder model."
        }
...
async def switch_to_local_inference(enable: bool = True) -> Dict[str, Any]:
    ...
    logger.info(f"ADE status: Switching inference mode to {'LOCAL' if enable else 'CLOUD'}")
