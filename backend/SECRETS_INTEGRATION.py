...
def validate_with_providers():
    ...
    try:
        from secrets_validator import validate_secrets
...
1. VERIFY SECRETS ARE LOADED
   Command: python -c "from config import get_settings; s = get_settings(); s.validate()"
   Expected: All critical API keys validated and loaded

2. TEST THE APPLICATION STARTUP
   Command: python -c "from main import app; print('App ready')"
