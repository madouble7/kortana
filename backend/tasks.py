...
def process_chat(self, message: str, conversation_id: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        # Use async function in sync context via asyncio.run
        import asyncio
        response = asyncio.run(gemini_service.analyze_text(message))
...
def analyze_image(self, image_url: str, prompt: str = "Analyze this image") -> dict[str, Any]:
    ...
    try:
        ...
        # Analyze with Gemini
        analysis = asyncio.run(gemini_service.analyze_multimodal(prompt, [image]))
...
"""
    ...
    classification = asyncio.run(gemini_service.analyze_text(prompt))
...
"""
    ...
    result = asyncio.run(gemini_service.analyze_text(prompt))
    return result
...
"""
    ...
    scaffold = asyncio.run(gemini_service.analyze_text(prompt))
    return scaffold
