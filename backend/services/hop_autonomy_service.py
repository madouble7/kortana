...
from tasks import run_autonomy_cycle
...
"""
            ...
            if classification not in ["auto", "ho", "approval"]:
                log_error("hop_autonomy", f"Invalid classification: {classification}, defaulting to 'ho'")
                classification = "ho"
    ...
    async def get_autonomy_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "last_run": recent_task.updated_at.isoformat() if recent_task and recent_task.updated_at else None,
    ...
    async def should_require_human(self, task: Task) -> bool:
        ...
        try:
            ...
            log_error("hop_autonomy", f"Error checking human requirement for task {task.id}: {str(e)}")
