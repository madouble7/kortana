...
def test_configuration():
    ...
    try:
        ...
        checks = [
            (
                "OpenAI API Key",
                bool(settings.OPENAI_API_KEY and len(settings.OPENAI_API_KEY) > 20),
            ),
            (
                "Anthropic API Key",
                bool(
                    settings.ANTHROPIC_API_KEY and len(settings.ANTHROPIC_API_KEY) > 20
                ),
            ),
        ...
        db_configured = (
            ...
            or "postgres" in settings.DATABASE_URL
        )
        print_result(
