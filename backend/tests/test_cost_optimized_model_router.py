...
import pytest
from src.kortana.api_integration import ProviderRateLimitError, UnifiedAPIClient
...
def test_summary_tasks_enable_openai_fast_lane(monkeypatch) -> None:
    ...
    assert providers[0] == ModelProvider.GROQ
...
def test_cost_router_uses_gemini_service_model_selection(monkeypatch) -> None:
    ...
    assert router.configs[ModelProvider.GEMINI].model_name == "gemini-2.5-flash-lite"
...
@pytest.mark.asyncio
@patch("src.kortana.api_integration.get_cost_optimized_model_router")
async def test_unified_api_client_cools_down_rate_limited_provider(
    mock_get_router,
) -> None:
    mock_router = mock_get_router.return_value
    mock_router.configs = {}
