...
Synchronized with Transcendent Schema & Optimization Suite
"""
...
def verify_stripe_configured():
    ...
    if not secret_key or secret_key in ["", "None", "your-secret-key-here"]:
        raise HTTPException(
            status_code=503,
...
async def create_customer(customer_data: CustomerCreate):
    ...
    try:
        ...
        logger.error(f"Billing system error: {str(e)}")
        raise HTTPException(
...
async def create_subscription(subscription_data: SubscriptionCreate):
    """Create a new subscription with idempotency lock"""
    verify_stripe_configured()
        ...
        if subscriptions.data:
            ...
            if subscription.metadata and "plan_type" in subscription.metadata:
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid plan type in subscription metadata: {plan_type_str}"
                    )
