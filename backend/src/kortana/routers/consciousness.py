...
# read from RevelationMemory rows written by the revelation engine.
# All public endpoints below are READ-ONLY.
# ==================================================================
...
async def get_wisdom(
    ...
    from src.kortana.models import RevelationMemory

    stmt = (
        select(RevelationMemory)
        .where(
            RevelationMemory.revelation_type.in_(
                ["pattern", "contradiction", "self_discovery"]
            )
        )
...
async def get_predictions(
    ...
    stmt = (
        select(RevelationMemory)
        .where(RevelationMemory.revelation_type == "prediction")
        .order_by(RevelationMemory.created_at.desc())
        .limit(limit)
    )
    result = await db.execute(stmt)
