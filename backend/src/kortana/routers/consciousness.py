...
# ------------------------------------------------------------------
# Phase 4: Wisdom & Prediction endpoints (READ-ONLY)
#
# Wisdom distillation and prediction generation are fully autonomous —
# driven exclusively by the Silent Reviewer daemon loop.
# These endpoints exist only for observing accumulated wisdom.
# ------------------------------------------------------------------
@router.get("/wisdom")
async def get_wisdom(
    ...
    """Return stored wisdom entries.

    Wisdom is self-generated by the autonomous Silent Reviewer daemon.
    This endpoint is read-only — there is no manual trigger.
    """
...
async def get_predictions(
    ...
    Predictions are self-generated by the autonomous Silent Reviewer daemon.
    This endpoint is read-only — there is no manual trigger.
    """
    from sqlalchemy import select
...
# ------------------------------------------------------------------
# Internal autonomous cycle endpoint (called ONLY by Silent Reviewer)
# ------------------------------------------------------------------
@router.post("/_internal/wisdom-cycle")
async def internal_wisdom_cycle(
    db: AsyncSession = Depends(get_db),
) -> Dict[str, Any]:
    """Internal endpoint for the Silent Reviewer daemon.

    NOT intended for manual use. The daemon is the single source of truth
    for wisdom distillation and prediction generation.
    """
    engine = RevelationEngine(db)
    wisdom = await engine.distill_wisdom()
    predictions = await engine.predict_evolution()
    return {
