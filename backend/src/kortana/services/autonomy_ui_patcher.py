"""
KOR'TANA Autonomous UI Patcher — Phase 8
...
class AutonomyUIPatcher:
    """Autonomous patching engine for self-repairing KOR'TANA UI."""
    ...
    FRONTEND_VERIFICATION_COMMANDS = (
        VerificationCommand(
            "install",
            ("npm.cmd" if __import__("os").name == "nt" else "npm", "install"),
        ),
        VerificationCommand(
            "lint",
            ("npm.cmd" if __import__("os").name == "nt" else "npm", "run", "lint"),
        ),
        VerificationCommand(
            "type-check",
            (
        ...
        VerificationCommand(
            "build",
            ("npm.cmd" if __import__("os").name == "nt" else "npm", "run", "build"),
        ),
    )
    ...
    def __init__(self, db: AsyncSession) -> None:
        ...
        self.worktree_dir = self.boundary.resolve_canonical_path(
            self.repo_root / ".autonomy_ui_patcher_worktree"
        )
    ...
    async def attempt_auto_fix(
        ...
        4. Run npm install, lint, type-check, and build
        ...
        try:
            ...
            new_content = self._extract_code_content(new_content_response)
    ...
    def _build_patch_prompt(
        ...
        *,
        ...
        return (
            ...
            f"```\n{file_content}\n```\n\n"
            "Provide the COMPLETE correct file content that applies the fix. "
            "Respond ONLY with the complete code in a code block, "
    ...
    def _resolve_target_file(self, target_file: str) -> Path:
        ...
        try:
            candidate.relative_to(self.repo_root / "frontend")
        except ValueError as exc:
            raise ValueError(
                "AutonomyUIPatcher currently supports frontend files only."
            ) from exc

        return candidate

    @staticmethod
    def _extract_code_content(new_content_response: str) -> str:
    ...
    async def _run_verification_suite(
        ...
        for command in self.FRONTEND_VERIFICATION_COMMANDS:
    ...
    def _verification_root(self, relative_target: Path) -> Path:
        if not relative_target.parts or relative_target.parts[0] != "frontend":
            raise ValueError(
                "AutonomyUIPatcher currently supports frontend files only."
            )
        return self.worktree_dir / "frontend"
    ...
    async def _commit_worktree_change(
        ...
        *,
        ...
        commit_message = (
            f"fix(auto-repair-ui): resolve {error_type}\n\nRoot Cause: {root_cause}"
        )
    ...
    async def _run_process(
        ...
        *,
        ...
        return await asyncio.to_thread(
            ...
            text=True,
            check=False,
            # Need shell=True on Windows for npm? Using standard subprocess for npm commands can be tricky on Windows depending on how git bash or pwsh behaves, but we will leave arguments matching the Python ones and trust the system's PATH.
            shell=True if argv[0] == "npm" else False,
        )
