...
def ho_1_github_token() -> str | None:
...
def ho_3_create_database(db_password: str | None = None) -> bool:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="KOR'TANA Autonomous Execution System")

    parser.add_argument(
        "--all", action="store_true", help="Execute all automatable steps (HO-3 through HO-8)"
    )
    parser.add_argument("--create-db", action="store_true", help="Execute HO-3: Create database")
    parser.add_argument("--populate-env", action="store_true", help="Execute HO-4: Populate .env")
    parser.add_argument("--run-migration", action="store_true", help="Execute HO-5: Run migration")
    parser.add_argument(
        "--install-deps", action="store_true", help="Execute HO-6: Install dependencies"
    )
    parser.add_argument("--start-server", action="store_true", help="Execute HO-7: Start server")
    parser.add_argument("--verify-health", action="store_true", help="Execute HO-8: Verify health")
    parser.add_argument(
        "--interactive", action="store_true", help="Run in interactive mode (confirm each step)"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would execute without running"
    )
    parser.add_argument(
        "--github-token", type=str, help="Provide GitHub token directly (not recommended)"
    )
    parser.add_argument(
        "--gemini-key", type=str, help="Provide Gemini key directly (not recommended)"
    )
