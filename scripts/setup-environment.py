...
def setup_local_env():
    ...
    print("(You can get these from Google AI Studio and Google Cloud Console)")
    print()

    gemini_key = input("GEMINI_API_KEY (from Google AI Studio): ").strip()
    if not gemini_key:
        print("⚠️  No GEMINI_API_KEY provided - some features won't work")
...
def setup_cloud_run_secrets():
    ...
    print("📋 GCP Secret Manager setup:")
    print("(GCP Console → Security → Secret Manager)")
    print()
