#!/usr/bin/env python3
"""
Test script for Enhanced Model Router - Windows Compatible
...
def test_task_analysis():
    ...
    try:
        ...
        for user_input, expected_task in test_cases:
            ...
            status = "[PASS]" if detected_task == expected_task else "[FAIL]"
...
def test_deepseek_integration():
    ...
    try:
        ...
        # Check for any DeepSeek free model, not just specific ID
        deepseek_free_model = None
        for model_id in deepseek_models:
            if "free" in model_id:
                deepseek_free_model = model_id
                break

        if deepseek_free_model:
            print(f"Found DeepSeek free model: {deepseek_free_model}")
            model_info = router.get_model_info(deepseek_free_model)
            ...
            print("No DeepSeek free model found in configuration!")

    except Exception as e:
        logger.error(f"DeepSeek integration test failed: {e}")


def test_yaml_router_integration():
    """Test YAML Router integration."""
    ...
    try:
        ...
        print(f"YAML Router loaded {len(models)} models")
        ...
        if reasoning_model:
            print(
                f"Reasoning capability model: {reasoning_model.id} (${reasoning_model.cost_per_1m_input:.4f})"
            )

        # Test cheapest model selection
        cheapest_model = yaml_router.select_cheapest_model()
        if cheapest_model:
            print(
                f"Cheapest model: {cheapest_model.id} (${cheapest_model.cost_per_1m_input:.4f})"
            )

        # Test free models
        free_models = yaml_router.get_free_models()
        print(f"Free models available: {len(free_models)}")
        for model in free_models:
            print(f"  - {model.id}")

    except Exception as e:
        logger.error(f"YAML router integration test failed: {e}")
