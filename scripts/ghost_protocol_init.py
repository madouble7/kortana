...
from kortana.core.services import initialize_services, get_tabby_service
...
async def main():
    ...
    if not await tabby.check_availability():
        logger.error("Tabby executable not found. Please install Tabby (https://tabby.sh).")
        return
    ...
    if success:
        ...
        logger.error("Ghost Protocol: Model provisioning failed. Manual intervention required.")
