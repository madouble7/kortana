...
class AutonomousMonitor:
    ...
    def detect_state_changes(self, current_state: Dict) -> List[str]:
        ...
        for key, value in current_state.items():
            if key not in self.previous_state:
                ...
                changes.append(f"🔄 Changed {key}: {self.previous_state[key]} → {value}")
    ...
    def print_autonomous_activity(self):
        ...
        active_goals = [g for g in goals if g.get('status') not in ['COMPLETED', 'FAILED', 'CANCELLED']]
        ...
        if active_goals:
            ...
            for goal in active_goals:
                ...
                print(f"  {status_emoji} [{goal.get('id')}] {goal.get('title', 'Unknown')}")
                print(f"     Status: {goal.get('status', 'Unknown')}")
                print(f"     Created: {goal.get('created_at', 'Unknown')}")
                
                # Get tasks for this goal
                tasks = self.get_tasks_for_goal(goal.get('id'))
                if tasks:
                    ...
                    for task in tasks[-3:]:  # Show last 3 tasks
                        ...
                        print(f"        {task_emoji} {task.get('action_type', 'Unknown')}: {task.get('status', 'Unknown')}")
                print()
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"❌ Server not accessible: {e}")        print("🔧 Please start the server first:")
