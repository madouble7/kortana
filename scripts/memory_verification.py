...
class LearningVerifier:
    ...
    def get_memories(
        self, memory_type: str = None, limit: int = 50
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    "connections": graph_data.get("edges", [])[
                        :5
                    ],  # Sample connections
                }
            else:
    ...
    def verify_learning_progression(self) -> dict[str, Any]:
        ...
        for memory in all_memories:
            try:
                ...
                progression["memory_types"][memory_type] = (
                    progression["memory_types"].get(memory_type, 0) + 1
                )
    ...
    def print_learning_analysis(self):
        ...
        if belief_analysis["knowledge_domains"]:
            print(
                f"🎯 KNOWLEDGE DOMAINS ({len(belief_analysis['knowledge_domains'])}):"
            )
        ...
        if belief_analysis["confidence_scores"]:
            avg_confidence = sum(belief_analysis["confidence_scores"]) / len(
                belief_analysis["confidence_scores"]
            )
            print("\n📊 CONFIDENCE METRICS:")
            print(f"   Average Confidence: {avg_confidence:.2f}")
            print(
                f"   High Confidence (>0.8): {sum(1 for c in belief_analysis['confidence_scores'] if c > 0.8)}"
            )
            print(
                f"   Low Confidence (<0.5): {sum(1 for c in belief_analysis['confidence_scores'] if c < 0.5)}"
            )
        ...
        if graph_info["status"] == "available":
            ...
            print(
                f"   Status: {graph_info['status']} ({graph_info.get('reason', 'Unknown')})"
            )
    ...
    def verify_autonomous_learning(self) -> bool:
        ...
        if new_memories > 0:
            print(
                f"✅ AUTONOMOUS LEARNING CONFIRMED: {new_memories} new memories formed!"
            )
            return True
        else:
            print(
                "❌ No new memories detected - may indicate lack of autonomous activity"
            )
            return False
