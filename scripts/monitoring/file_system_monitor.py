        ...
        for dir_path in self.watch_dirs:
            if dir_path.endswith('.txt') or dir_path.endswith('.toml'):
                ...
                for root, dirs, files in os.walk(dir_path):
                    for file in files:
                        if file.endswith(('.py', '.txt', '.md', '.yaml', '.yml', '.json', '.toml')):
        ...
        if modified_files:
            ...
            for change in modified_files:
                ...
                if "planning_engine" in file_path:
                    ...
                    print("      ⚡ Execution engine modification - capability expansion")
                elif "memory" in file_path or "learning" in file_path:
                    print("      🧠 Memory/Learning system modification - knowledge formation")
            ...
            while True:
                ...
                if changes:
                    ...
                    if int(time.time()) % 30 == 0:
                        print(f"📁 Monitoring... ({len(self.baseline)} files, {self.change_count} total changes)")
