...
try:
    ...
    from kortana.voice import VoiceChatOrchestrator, STTService, TTSService
...
async def on_guild_join(guild):
    ...
    if guild.system_channel:
        embed = discord.Embed(
            ...
            description=("Hello! I'm Kor'tana, your AI companion.\n\n"
                        "**Text Commands:**\n"
...
async def kortana_chat(interaction: discord.Interaction, message: str):
    ...
    try:
        ...
        await interaction.followup.send(
            f"⚠️ I'm having trouble processing that. Error: {str(e)}",
            ephemeral=True
        )
...
async def join_voice(interaction: discord.Interaction):
    ...
    if not voice_orchestrator:
        await interaction.response.send_message(
            "⚠️ Voice features are currently unavailable.",
            ephemeral=True
        )
        return
    ...
    try:
        ...
        voice_sessions[interaction.guild.id] = {
            'channel': channel,
            'users': set()
        }
    ...
    if bot.user.mentioned_in(message) and not message.mention_everyone:
        ...
        try:
            if chat_engine:
                ...
                response = f"Hello {message.author.display_name}! I heard you mention me."
