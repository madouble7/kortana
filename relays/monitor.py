...
class KortanaEnhancedMonitor:
    ...
    def log_token_usage(self, task_id: str, stage: str, tokens: int, agent_name: str = "system"):
    ...
    def log_chain_communication(self, task_id: str, stage: str, tokens: int, agent_from: str, agent_to: str):
        ...
        try:
            ...
            cursor.execute(
                ...
                (task_id, stage, tokens, datetime.utcnow().isoformat(), agent_from, agent_to)
            )
            conn.commit()
            conn.close()
        except Exception as e:
    ...
    def get_active_agents(self) -> List[str]:
        ...
        try:
            ...
            cursor.execute("SELECT DISTINCT agent_name FROM token_log WHERE timestamp > ?", (one_hour_ago,))
    ...
    def get_token_usage_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute("SELECT stage, SUM(tokens) FROM token_log WHERE timestamp > ? GROUP BY stage", (one_day_ago,))
            stats["last_24h"] = {stage: tokens for stage, tokens in cursor.fetchall()}

            # Last hour usage
            one_hour_ago = (datetime.utcnow() - timedelta(hours=1)).isoformat()
            cursor.execute("SELECT stage, SUM(tokens) FROM token_log WHERE timestamp > ? GROUP BY stage", (one_hour_ago,))
            stats["last_hour"] = {stage: tokens for stage, tokens in cursor.fetchall()}

            # Usage by agent
            cursor.execute("SELECT agent_name, SUM(tokens) FROM token_log WHERE timestamp > ? GROUP BY agent_name", (one_day_ago,))
            ...
            cursor.execute("SELECT SUM(tokens) FROM token_log WHERE timestamp > ?", (today_start,))
    ...
    def get_rate_limit_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            one_hour_ago = (datetime.utcnow() - timedelta(hours=1)).isoformat()
            cursor.execute("SELECT SUM(tokens) FROM token_log WHERE timestamp > ? AND stage IN ('summarize', 'init', 'process')", (one_hour_ago,))
            result = cursor.fetchone()
            limits["gemini_flash"]["used_tokens"] = result[0] if result[0] else 0

            # Calculate request counts
            cursor.execute("SELECT COUNT(*) FROM token_log WHERE timestamp > ?", (one_hour_ago,))
            result = cursor.fetchone()
            limits["gemini_flash"]["used_requests"] = result[0] if result[0] else 0

            # Calculate GitHub Models usage (daily)
            today_start = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0).isoformat()
            cursor.execute("SELECT COUNT(*) FROM chain_log WHERE timestamp > ? AND stage = 'test'", (today_start,))
            result = cursor.fetchone()
        ...
        for service, data in limits.items():
            ...
            if "tpm_limit" in data:
                data["tpm_percent"] = (data["used_tokens"] / data["tpm_limit"]) * 100 if data["tpm_limit"] > 0 else 0
            if "rpm_limit" in data:
                data["rpm_percent"] = (data["used_requests"] / data["rpm_limit"]) * 100 if data["rpm_limit"] > 0 else 0
            if "requests_limit" in data:
                data["requests_percent"] = (data["used_requests"] / data["requests_limit"]) * 100 if data["requests_limit"] > 0 else 0
    ...
    def get_context_window_status(self) -> Dict[str, Any]:
        ...
        for model, limit in context_limits.items():
            ...
            status[model] = {
                ...
                "status": "optimal" if percent_used < 70 else "warning" if percent_used < 90 else "critical"
            }
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute("SELECT timestamp FROM token_log ORDER BY timestamp DESC LIMIT 1")
            result = cursor.fetchone()
            if result:
        ...
        if active_agents > 0:
            ...
            try:
                last_time = datetime.fromisoformat(health["last_activity"].replace('Z', ''))
    ...
    def print_dashboard(self):
        ...
        for service, data in rate_limits.items():
            ...
            if "rpm_limit" in data:
                print(f"           RPM: {data['used_requests']}/{data['rpm_limit']} ({data['rpm_percent']:.1f}%)")
            if "requests_limit" in data:
                print(f"           Daily: {data['used_requests']}/{data['requests_limit']} ({data['requests_percent']:.1f}%)")

        # Context Windows
        context_windows = dashboard["context_windows"]
        print(f"\n[CONTEXT] Context Window Status:")
        for model, data in context_windows.items():
            status_indicator = "[OK]" if data["status"] == "optimal" else "[WARN]" if data["status"] == "warning" else "[CRIT]"
            print(f"         {model}: {data['used']:,}/{data['limit']:,} tokens ({data['percent_used']:.1f}%) {status_indicator}")

        print("\n" + "=" * 80)

    def monitor_loop(self, interval: int = 300):
        """Run continuous monitoring loop (default 5 minutes)"""
        print(f"[MONITOR] Starting enhanced monitoring dashboard...")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "--loop":
            ...
            monitor.log_chain_communication("dialogue_parser_001", "handoff", 1200, "claude", "weaver")
