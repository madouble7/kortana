...
import { GitHubConnector } from '../services/GitHubConnector.ts';
import { KortanaAI } from '../services/KortanaAI.ts';
 ...
 */
router.post('/webhook', express.raw({ type: 'application/json', verify: verifySignature }), async (req: Request, res: Response) => {
  ...
  try {
    switch (event) {
      case 'push':
        ...
        break;
      case 'issues':
        console.log(`Issue ${payload.action}: ${payload.issue.html_url}`);
        break;
      case 'pull_request':
