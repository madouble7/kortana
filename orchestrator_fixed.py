...
class KorOrchestrator:
    def __init__(self, db: Session, config: KortanaConfig | None = None):
        ...
        self.default_model_id = models_config.get('default', {}).get('model', 'gpt-4.1-nano')
    ...
    def _load_models_config(self) -> dict:
        ...
        config_paths = [
            ...
            Path("../config/models_config.json"),
            Path(__file__).parent.parent.parent.parent / "config" / "models_config.json",
        ]
    ...
    async def process_query(self, query: str) -> dict[str, Any]:
        ...
        if not llm_client:
            error_message = f"Failed to initialize LLM client for model {self.default_model_id}"
        ...
        print(f"--- Final Kor'tana Response ---\n{final_response}")  # Log final response
