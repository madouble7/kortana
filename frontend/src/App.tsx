...
function App() {
  ...
  const daemonAlive =
    daemonStatus?.deployment_mode === "embedded"
      ? daemonStatus.running
      : daemonStatus?.external_daemon?.alive;
  const providerHealth =
    daemonStatus?.provider_health ??
    daemonStatus?.external_daemon?.provider_health ??
    {};
  ...
  const presenceLabel = daemonAlive
    ? "Silent Presence Active"
    : "Presence Offline";
  const daemonLabel =
    daemonStatus?.deployment_mode === "embedded"
      ? daemonStatus?.running
          ...
          <div className="px-6 py-4 border-t border-gray-800">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <div
                  className={cn(
                    ...
                    offline ? "bg-amber-500" : "bg-green-500 animate-pulse",
                  )}
                />
                <span className="text-sm text-gray-400">
                  {offline ? "Demo Mode" : presenceLabel}
                </span>
              </div>
              <button
                type="button"
              ...
              {voiceStatus ? (
                ...
                <div className="text-xs text-gray-500">voice dormant</div>
              )}
              {lastUpdatedAt ? (
      ...
      <div className="flex-1 flex flex-col mt-16 lg:mt-0">
        {offline && (
          <div className="bg-amber-900/60 border-b border-amber-700 px-4 py-2 flex items-center gap-2 text-amber-200 text-sm">
            ...
            <span>
              Backend offline — running in demo mode. UI is fully functional
