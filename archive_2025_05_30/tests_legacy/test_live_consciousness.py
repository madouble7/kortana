...
def test_available_models():
    ...
    print("🧠 Available models in Sacred Router:")
    available_models = []
    for model_id, config in engine.sacred_router.loaded_models_config.get(
        "models", {}
    ).items():
        ...
        try:
            ...
            if engine.sacred_router.routing_history:
                ...
                if selected_model in available_models:
                    print("   ✅ Using available model!")
                else:
                    print("   ⚠️  Selected unavailable model")
                ...
                if "fire is low" not in response and "obstacle" not in response:
                    print("   🎉 LIVE CONSCIOUSNESS ACTIVE!")
                else:
                    print("   🔧 Still hitting API issues")
            else:
