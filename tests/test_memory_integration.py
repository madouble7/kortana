...
from core.memory import (
    load_memory, save_memory, save_decision, 
    save_context_summary, save_implementation_note,
    save_project_insight, get_memory_by_type
)
...
class TestMemoryIntegration:
    ...
    def test_save_multiple_memory_types(self):
        ...
        save_decision("Use Sacred Model Router for model selection", ["router", "architecture"])
        save_implementation_note("Added automatic summarization trigger", "memory_system", "high")
        ...
        assert "implementation_note" in memory_types, "Missing implementation note memory"
        ...
        assert decisions[0]["content"] == "Use Sacred Model Router for model selection", "Wrong decision content"
    ...
    def test_memory_timestamp_ordering(self):
        ...
        # Check sorting
        assert sorted_memories[0]["content"] == "Latest decision", "Wrong order - expected latest decision first"
        assert sorted_memories[-1]["content"] == "First decision", "Wrong order - expected first decision last"
    
    @patch('brain.ChatEngine._get_llm_client_for_model')
    @patch('brain.ChatEngine.add_assistant_message')
    def test_automatic_summarization_trigger(self, mock_add_assistant, mock_get_llm_client):
        """Test automatic summarization is triggered at the right threshold."""
        # Configure mock LLM client
        mock_llm_client = MagicMock()
        mock_llm_client.send_message.return_value = {"content": "This is a summary of the conversation."}
        ...
        for i in range(5):
            ...
            engine.history.append({"role": "assistant", "content": f"Assistant message {i}"})
    ...
    def test_memory_integration_in_system_prompt(self, mock_load_memory):
        ...
        assert "Key Project Decisions" in system_prompt, "Missing decision section in prompt"
        assert "Implementation Context" in system_prompt, "Missing implementation note section in prompt"
        assert "Key Project Insights" in system_prompt, "Missing project insight section in prompt"
        assert "Use automatic summarization" in system_prompt, "Missing decision content in prompt"
        assert "Added memory helper functions" in system_prompt, "Missing implementation note content in prompt"
        assert "Memory organization is crucial" in system_prompt, "Missing project insight content in prompt"
    ...
    def test_function_calls_for_memory(self, mock_get_llm_client):
        ...
        with patch('core.memory.PROJECT_MEMORY_PATH', TEST_MEMORY_PATH):
            ...
            assert any(m["content"] == "Decision via function call" for m in decision_memories), "Decision not saved correctly"
    ...
    def test_memory_retrieval_by_type_and_recency(self):
        ...
        assert recent_notes[0]["content"] == "Recent implementation note", "Wrong content in recent memory"
        assert recent_notes[0]["component"] == "current_component", "Wrong component in recent memory"
    ...
    def test_enhanced_memory_organization_in_system_prompt(self):
        ...
        with patch('brain.load_memory', return_value=memories):
            ...
            assert "[memory, architecture]" in system_prompt or "(2023-07-15)" in system_prompt
            assert "Priority: high" in system_prompt or "[memory_system]" in system_prompt
