...
from core.memory import (
    save_memory, load_memory, save_decision, 
    save_context_summary, save_implementation_note,
    save_project_insight, get_memory_by_type, get_recent_memories_by_type
)
...
class TestMemoryHelpers:
    ...
    def mock_memory_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w+', delete=False) as temp:
            temp.write('{"type": "decision", "content": "Test decision", "timestamp": "2023-06-15T12:00:00Z", "tags": ["test"]}\n')
            temp.write('{"type": "implementation_note", "content": "Test note", "timestamp": "2023-06-16T12:00:00Z", "component": "test", "priority": "high"}\n')
            temp_name = temp.name
    ...
    def test_load_memory(self, mock_memory_file):
        ...
        assert memories[0]["content"] == "Test decision", "Wrong content for first memory"
        assert memories[1]["type"] == "implementation_note", "Wrong type for second memory"
    ...
    def test_save_memory(self):
        ...
        with patch('builtins.open', m):
            with patch('os.makedirs'):
                ...
                assert json.dumps(memory) in written_content, "Memory entry should be written to file"
    ...
    def test_save_decision(self):
        ...
        with patch('core.memory.save_memory') as mock_save:
            ...
            assert call_args["content"] == "Test decision", "Wrong content in saved memory"
    ...
    def test_save_implementation_note(self):
        ...
        with patch('core.memory.save_memory') as mock_save:
            ...
            mock_save.assert_called_once()
            call_args = mock_save.call_args[0][0]
            assert call_args["type"] == "implementation_note", "Wrong type in saved memory"
            assert call_args["content"] == "Test note", "Wrong content in saved memory"
            assert call_args["component"] == "test_component", "Wrong component in saved memory"
    ...
    def test_save_project_insight(self):
        ...
        with patch('core.memory.save_memory') as mock_save:
            ...
            assert call_args["content"] == "Test insight", "Wrong content in saved memory"
    ...
    def test_get_memory_by_type(self, mock_memory_file):
        ...
        assert decision_memories[0]["content"] == "Test decision", "Wrong content in decision memory"
        ...
        assert note_memories[0]["content"] == "Test note", "Wrong content in implementation note memory"
    ...
    def test_get_recent_memories_by_type(self, mock_memory_file):
        ...
        assert recent_decisions[0]["content"] == "Test decision", "Wrong content in recent decision memory"
