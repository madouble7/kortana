"""
Standalone tests for optimization module inspired by RebelBrowser/Chromium.
"""
...
# Add the optimization module directly to path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../src/kortana/core'))

# Import directly without triggering kortana core __init__
...
class TestMemoryOptimizer(unittest.TestCase):
    ...
    def test_lru_cache_basic(self):
        ...
        optimizer = mem_opt.MemoryOptimizer(cache_capacity=3)
    ...
    def test_lru_cache_eviction(self):
        ...
        optimizer = mem_opt.MemoryOptimizer(cache_capacity=2)
        cache = optimizer.get_cache("test")
    ...
    def test_cache_stats(self):
        ...
        optimizer = mem_opt.MemoryOptimizer(cache_capacity=5)
        cache = optimizer.get_cache("test")
    ...
    def test_memory_pool(self):
        ...
        optimizer = mem_opt.MemoryOptimizer()
...
class TestResourceManager(unittest.TestCase):
    ...
    def test_resource_pool_acquire_release(self):
        ...
        pool = res_mgr.ResourcePool("test", factory=factory, min_size=2, max_size=5)
    ...
    def test_resource_pool_max_size(self):
        ...
        pool = res_mgr.ResourcePool("test", factory=factory, min_size=1, max_size=3)
    ...
    def test_resource_cleanup(self):
        ...
        pool = res_mgr.ResourcePool(
            "test", factory=factory, cleanup=cleanup, min_size=1, max_size=2
        )

        # Acquire and release more resources than pool can hold
        resources = [pool.acquire() for _ in range(3)]
        
        # Release all - pool can only hold 2, so 1 should be cleaned up
        for resource in resources:
            pool.release(resource)

        # At least one resource should have been cleaned up
        self.assertGreaterEqual(len(cleanup_called), 1)

    def test_resource_manager(self):
        """Test resource manager coordination."""
        manager = res_mgr.ResourceManager()
...
class TestPerformanceMetrics(unittest.TestCase):
    ...
    def test_counter_metrics(self):
        ...
        metrics = perf_met.PerformanceMetrics("test")
    ...
    def test_timer_context_manager(self):
        ...
        with perf_met.Timer(metrics, "operation"):
            time.sleep(0.01)
    ...
    def test_metrics_collector(self):
        ...
        collector = perf_met.MetricsCollector()
...
class TestPriorityQueue(unittest.TestCase):
    ...
    def test_priority_queue_ordering(self):
        ...
        queue = prio_queue.PriorityQueue("test")

        queue.enqueue(lambda: None, priority=prio_queue.Priority.LOW, task_id="low")
        queue.enqueue(
            lambda: None, priority=prio_queue.Priority.CRITICAL, task_id="critical"
        )
        queue.enqueue(
            lambda: None, priority=prio_queue.Priority.NORMAL, task_id="normal"
        )
    ...
    def test_task_processor(self):
        ...
        queue = prio_queue.PriorityQueue("test")
        processor = prio_queue.TaskProcessor(queue, num_workers=2, name="test")
        ...
        queue.enqueue(task_func, prio_queue.Priority.NORMAL, "task1", 1)
        queue.enqueue(task_func, prio_queue.Priority.NORMAL, "task2", 2)
        queue.enqueue(task_func, prio_queue.Priority.NORMAL, "task3", 3)
    ...
    def test_decision_queue(self):
        """Test decision queue."""
        decision_queue = prio_queue.DecisionQueue(num_workers=2)
        ...
        decision_queue.submit_decision(
            decision_func, prio_queue.Priority.HIGH, "decision1", "dec1"
        )
        decision_queue.submit_decision(
            decision_func, prio_queue.Priority.NORMAL, "decision2", "dec2"
        )
    ...
    def test_queue_stats(self):
        ...
        queue.enqueue(lambda: None, prio_queue.Priority.NORMAL, "task1")
        queue.enqueue(lambda: None, prio_queue.Priority.HIGH, "task2")
...
class TestIntegration(unittest.TestCase):
    ...
    def test_full_optimization_stack(self):
        ...
        # Setup
        optimizer = mem_opt.MemoryOptimizer(cache_capacity=10)
        resource_manager = res_mgr.ResourceManager()
        collector = perf_met.MetricsCollector()
        decision_queue = prio_queue.DecisionQueue(num_workers=2)
        ...
        decision_queue.submit_decision(decision, prio_queue.Priority.HIGH, "test", 42)
